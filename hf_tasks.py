"""
    ...
    VLLM was problematic when using SmolLM3 because of the lack of a main branch with config, tokenizer, etc.
"""
...
def main(_):
    ...
    if cfg.quantize:
        if not os.path.exists(saved_quantized):
            ...
            raise ValueError(f"Quantized model not found at {saved_quantized}. Cannot proceed.")
        model_path = saved_quantized
        kwargs = {'tokenizer': cfg.tokenizer_name}
    else:
        model_path = cfg.repo_origin
        kwargs = {'revision': cfg.revision, 'trust_remote_code':True}
